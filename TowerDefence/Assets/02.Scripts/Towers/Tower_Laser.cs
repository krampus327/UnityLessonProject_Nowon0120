    ...
    private void Laser()
    {
        ...
        else
        {
            if(targetEnemy != target.GetComponent<Enemy>())
            {
                ...
                buffCoroutine = StartCoroutine(BuffManager.ActiveBuff(BuffManager.ActiveBuff(target.GetComponent<Enemy>()), buffSlow));
            ...
            targetEnemy.hp -= (int)(damage * (1f + damageIncrementPercent / 100) * (damageStep + 1) * Time.deltaTime);
