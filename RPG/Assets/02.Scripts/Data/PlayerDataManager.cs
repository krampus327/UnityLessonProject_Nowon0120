...
public class PlayerDataManager
{
    ...
    public static string dirPath; // 플레이어 데이터를 저장할 디렉토리 경로, 생성자 호출 전에 참조하면 안됨.
    ...
    {
        dirPath = $"{Application.persistentDataPath}/PlayerData"; // Application.persistentDataPath는 운영체제마다 다름.
    }
    ...
    public static PlayerData CreateData(string nickName)
    {
        ...
        string jsonPath = dirPath + $"/Player_{nickName}.json"; // 플레이어 데이터 객체를 serialize해서 저장할 json 포멧 텍스트 파일 이름
        string jsonData = JsonUtility.ToJson(data); // 플레이어 데이터 Serialize 시켜서 문자열로 변환
        System.IO.File.WriteAllText(jsonPath, jsonData); // Serialize 된 플레이어 데이터를 파일 경로에다가 파일 생성
    ...
    {
        string[] jsonPaths = System.IO.Directory.GetFiles(dirPath); // 플레이어 데이터가 저장되는 경로세어 모든 플레이어 데이터 파일 경로 읽어옴.
    ...
    public static void SaveData()
    {
        ...
        string jsonData = JsonUtility.ToJson(_data); // 플레이어 데이터 Serialize 해서 json 포멧의 문자열로 변환
        System.IO.File.WriteAllText(jsonPath, jsonData); // 변환된 json 포멧의 문자열을 파일로 씀.
    }
